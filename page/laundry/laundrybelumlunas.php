        ...
        <div class="card m-b-30">
          ...
          <div class="table-responsive">
            ...
            <table id="datatable" class="table table-bordered">
              ...
              $query = "SELECT * FROM `tb_transaksi` INNER JOIN `tb_pelanggan` ON `tb_transaksi`.`pelangganid` = `tb_pelanggan`.`pelangganid` INNER JOIN `tb_users` ON `tb_users`.`userid` = `tb_transaksi`.`userid` INNER JOIN `tb_jenis` ON `tb_jenis`.`kd_jenis` = `tb_transaksi`.`kd_jenis` WHERE `tb_transaksi`.`status_pembayaran` = 0";
