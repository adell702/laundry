<?php
  ...
  $query = "SELECT * FROM `tb_transaksi` INNER JOIN `tb_pelanggan` ON `tb_transaksi`.`pelangganid` = `tb_pelanggan`.`pelangganid` INNER JOIN `tb_users` ON `tb_users`.`userid` = `tb_transaksi`.`userid` INNER JOIN `tb_jenis` ON `tb_jenis`.`kd_jenis` = `tb_transaksi`.`kd_jenis` WHERE `tb_transaksi`.`id_laundry` = '$id_laundry'";
...
<table width='100%'>
  <tr>
    <td>
        Jl. Jatimulya Raya No. 98, Jatimulya, Tambun Selatan, Bekasi <br>
        No. Hp / WA : 089652137823 <br>
        Email : triplealaundry@gmail.com <br>
        Jam Operasional : Senin – Minggu : 09.30 – 20.30 WIB
    </td>
    <td align="right">
...
<hr style="border:1; border-top: 5px double #8c8c8c;">
